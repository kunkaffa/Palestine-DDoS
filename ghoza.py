...
def countdown(t):
    ...
    while True:
        ...
        if remaining_time > 1:
            ...
            print(f"\r🚀 {Fore.WHITE} {url} {Fore.CYAN} request {request}")
        else:
            stdout.flush()
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=conn) as session:
        ...
        async def progress():
            while time.time() < end_ts:
                ...
                print(f"\r☠️ {Fore.YELLOW} {url} {Fore.GREEN}r_limit {Fore.MAGENTA} {rate_limit}")
                print(f"\r🚀 {Fore.WHITE} {url} {Fore.CYAN} request {request}")
...
def confirm_ethical_use(target_host: str) -> bool:
    ...
    print(f"{Fore.YELLOW}└> Type 'reload' to continue: {' ' * 35}")
